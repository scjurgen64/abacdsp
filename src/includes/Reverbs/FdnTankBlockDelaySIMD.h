...
class FdnTankBlockDelaySIMD
{
  public:
    ...
    explicit FdnTankBlockDelaySIMD(const float sampleRate)
    ...
    void matrixFeed(size_t idx)
    {
        ...
        {
            hadamardFeed4_simd(m_lastValue[idx].data(), m_feedValue[idx].data());
        }
        if constexpr (ORDER == 8)
        {
            hadamardFeed8_simd(m_lastValue[idx].data(), m_feedValue[idx].data());
        }
        if constexpr (ORDER == 16)
        {
            hadamardFeed16_simd(m_lastValue[idx].data(), m_feedValue[idx].data());
        }
        if constexpr (ORDER == 32)
        {
            hadamardFeed32_simd(m_lastValue[idx].data(), m_feedValue[idx].data());
        }
