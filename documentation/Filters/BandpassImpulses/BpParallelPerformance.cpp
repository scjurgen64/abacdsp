...
PerformanceResult perf(const std::string& name, size_t NumElements)
{
    ...
    const size_t runs = 200'000'000 / NumElements;

    IMPL sut{48000.f};
    for (size_t i = 0; i < NumElements; ++i)
    {
        sut.setByDecay(i, 0, 1000.f + static_cast<float>(i) * 42.f, 10.f);
    }
...
int main(int argc, char* argv[])
{
    ...
    runPerfTests(results, std::integer_sequence<size_t, 4, 8, 12, 16, 32, 64, 100, 128, 256>{});
